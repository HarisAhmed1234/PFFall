 ...
 * Name: Haris Ahmed
 * Date: 18/10/23
 * Description: Input mobile phone pricing, calculate bill from each branch, 
 *              and output maximum bill, branch id, and phone ids where it occurred
 */
...
void inputBranches(int *arr, int n, int m) {
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			printf("Enter the price of mobile phone #%d-%d: ", i, j);
			scanf("%d", (arr + i*n + j));
		}
	}
